...
Use this when the user asks to save tokens, control context, avoid loading too many files, or make Forge output current-agent role work orders with a predictable context budget.
...
- AI Orchestration / Single-Host Role Work: include task goal, relevant files, acceptance, and verification; avoid broad background.
- Multi-Agent Collaboration: each role lane receives only its own task card and shared index pointers.

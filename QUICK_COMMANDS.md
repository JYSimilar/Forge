...
## Forge 2.2 Stable Core / 稳定内核

```text
Forge: 把这个任务写成 Safe Work Order，小步执行，省 token，停在 Review Gate。
```
...
```text
Forge: 让 Codex 调 Claude 做 review 这个做不到，给我人工交接方案。
```
...
```text
Forge: 这个仓库像 monorepo 吗？帮我识别 root、apps 和 packages。
```
...
```text
Forge: 跑一轮 field test，输出 FIELD_TEST_REPORT 和 field_test.json。
```

```text
Forge: 验证这套多智能体规划在当前项目上会不会卡住。
```
...
```text
Forge: 多角色之前先检查写入范围，避免两个角色同时改同一个文件。
```

```text
Forge: 更新 AGENT_INDEX，把 T1 标成 done，并记录验证证据。
```
...
```text
Forge: 给当前 agent 写一个边界清楚、低消耗的任务提示词。
```
...
```text
Forge: 把这个项目拆成任务队列，标出哪些交给人，哪些交给当前 agent 角色，哪些需要人工外部交接。
```

```text
Forge: 帮我写一份 AGENT_WORK_ORDER，先给当前 agent 角色执行；如需外部工具我会手动复制。
```
...
```text
Forge: 如果这一步适合找参考、定义完成标准、复盘或生成任务单，就轻量提醒我。
```
...
forge: write a current-agent work order

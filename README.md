...
Forge 1.7 is the stable project-manager skill release with Token Saver defaults, stronger existing-project self-audit, field-test feedback improvements, lightweight multi-agent collaboration planning, route chaining quality gates, Next Step Protocol, Capability Hints, Idea Backlog, Work Summary, and AI orchestration.
...
Forge 1.7 延续实战闭环：当你想验证 Forge 这套流程在真实项目上是否好用时，它会只读扫描 workspace，记录触发路线、证据、摩擦点、风险和下一步改进。它不会调用模型，也不会修改目标工程，并会把真实摩擦点转成下一轮小步优化。
...
Forge 1.7 只设计插件化路线，不实际拆分。未来可以把 Forge 拆成一个轻量 router skill，再拆出 audit、orchestration、review、docs、release、field-test 等子 skill 或 plugin 能力。

...
Forge 2.1 is the Agent-Compatible Work Protocol release: Token Saver by default, existing-project self-audit, field-test evidence, lightweight multi-agent collaboration planning, router contract regression, Dual Index human/machine indexes, Forge Doctor stability checks, Claude Code/Codex/Cursor-compatible work orders, route chaining quality gates, Next Step Protocol, Capability Hints, Idea Backlog, Work Summary, and AI orchestration.
...
- 校验自然触发、无感路由、最小 reference 加载和未来子 skill 交接契约。
- 同时生成 `FORGE_INDEX.md` 人类索引和 `forge_index.json` 机器索引，方便用户、AI、脚本和后续路由共享同一份事实。
- 发布或交付前运行 Forge Doctor，把工作区、路由、双索引、field test、agent index 和 release evidence 收束成稳定性总检。
- 规划未来插件化：先设计 router skill、子 skill 边界和迁移门禁，不急着拆实现。
...
- 生成 Claude Code / Codex / Cursor 都能消费的任务单，而不是重复宿主 agent 已有的调度、分支、review 或 tracing 功能。
...
2.0 的推荐路径是：先自查已有工程，再生成或更新双索引，然后用任务/多智能体/交付路线推进，最后用 Forge Doctor 做稳定性总检。小任务仍然走 Token Saver，不会默认生成报告或文件。
...
## Forge 2.1 Agent-Compatible Work Protocol

Forge 2.1 的边界是“项目管理协议层”：它不会声称自己能直接调用模型、自动调度宿主 agent、做 tracing 或替代 Claude Code / Codex / Cursor 的原生能力。它会把任务整理成跨宿主可用的 work order、验收标准、上下文预算和停手条件。

```text
Forge: 给 Claude Code 的任务单，要包含验收标准、上下文预算和不要碰的范围。
```

```text
Forge: 给 Codex 执行的 work order，只包含必要上下文。
```

```text
Forge: Claude Code / Codex / Cursor 怎么用这个任务？
```
...
Forge 2.0 仍然只设计插件化路线，不实际拆分。未来可以把 Forge 拆成一个轻量 router skill，再拆出 audit、orchestration、review、docs、release、field-test 等子 skill 或 plugin 能力。
...
Forge 2.0 在非小型项目任务中可以生成或更新双索引：`FORGE_INDEX.md` 给人看，`forge_index.json` 给 AI、脚本和后续路由看。两个文件来自同一份 payload，避免 Markdown 和 JSON 各说各话。
...
- `HOST_WORK_ORDER.md`：面向 Claude Code / Codex / Cursor / generic agent 的宿主兼容任务单。
...
如果你要继续参与迭代，可以留在 `main`；如果只是使用，优先选最新 tag。

...
Forge 1.9 is the stable project-manager skill release with Token Saver defaults, stronger existing-project self-audit, field-test feedback improvements, lightweight multi-agent collaboration planning, a router contract for natural triggers and future pluginization, Dual Index human/machine indexes, route chaining quality gates, Next Step Protocol, Capability Hints, Idea Backlog, Work Summary, and AI orchestration.
...
Forge 1.8 保留实战闭环：当你想验证 Forge 这套流程在真实项目上是否好用时，它会只读扫描 workspace，记录触发路线、证据、摩擦点、风险和下一步改进。它不会调用模型，也不会修改目标工程，并会把真实摩擦点转成下一轮小步优化。
...
Forge 1.8 仍然只设计插件化路线，不实际拆分。未来可以把 Forge 拆成一个轻量 router skill，再拆出 audit、orchestration、review、docs、release、field-test 等子 skill 或 plugin 能力。
...
Forge 1.8 不真正拆 skill，但把未来拆分前最容易乱的部分先做成可验证契约：自然触发词、路线 ID、最低读取 reference、默认输出、完成证据和交接路线。
...
Forge 1.9 在非小型项目任务中可以生成双索引：`FORGE_INDEX.md` 给人看，`forge_index.json` 给 AI、脚本和后续路由看。两个文件来自同一份 payload，避免 Markdown 和 JSON 各说各话。
